...
const SpecularButton = ({
  ...
  size = 'lg',
  radius = 999, // Changed default to pill
  tint = '#1D222D', // Default dark tint
  tintOpacity = 1,
  blur = 0,
  textColor = '#ffffff',
  lineColor = '#65C4EC', // KalpKrafts cyan highlight
  baseColor = '#2687E8', // KalpKrafts blue base stroke
  intensity = 1.2,
  shineSize = 15,
  shineFade = 60,
  thickness = 1.5,
  speed = 0.35,
  followMouse = true,
  proximity = 65, // Only fade in when very close/hovering
  autoAnimate = false, // Stop rotating idle animation
  disabled = false,
  onClick,
  className = '',
  type = 'button',
  ...
  useEffect(() => {
    ...
    ro.observe(btn);
    resize(btn.offsetWidth, btn.offsetHeight);
    ...
    const update = (now: number) => {
      ...
      const target = steer ? (pointerAngle ?? 0) : idleAngle;
