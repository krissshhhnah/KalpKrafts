...
export default function Loading() {
  return (
    <div className="flex min-h-screen w-full flex-col bg-[#F5FBFD] px-6 pt-32 pb-24 lg:px-12 xl:px-20 selection:bg-transparent">
      
      {/* Header Skeleton */}
      <div className="fixed top-0 left-0 right-0 z-50 flex h-16 items-center justify-between border-b border-[#D8EAF1]/50 bg-white/80 px-6 backdrop-blur-xl lg:px-8">
        <div className="h-8 w-36 animate-pulse rounded-lg bg-[#D8EAF1]/80" />
        <div className="hidden items-center gap-6 md:flex">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="h-4 w-16 animate-pulse rounded-md bg-[#D8EAF1]/50" />
          ))}
        </div>
        <div className="h-9 w-32 animate-pulse rounded-full bg-[#D8EAF1]/80" />
      </div>
